...
void PortA_Init(void){ 
  ...
  GPIOA->PUR = 0x0F;       				   // enable pullup resistors on first 4 pins      
  GPIOA->DEN = 0xFF;       				   // 7) enable digital pins PF4-PF0
	GPIOA->DATA = 0x00;
	
	// Setup the interrupt on PortA
	GPIOA->ICR = 0x0F;     // Clear any Previous Interrupt 
	GPIOA->IM |=0x0F;      // Unmask the interrupts for PF0 and PF4
	GPIOA->IS |= 0x0F;     // Make bits PF0 and PF4 level sensitive
	GPIOA->IEV &= ~0x0F;   // Sense on Low Level
...
void PortB_Init(void){ 
  ...
  GPIOA->AMSEL= 0x00;       				 // 3) disable analog function
  GPIOA->PCTL = 0x00000000;  				 // 4) GPIO clear bit PCTL  
  GPIOA->DIR = 0x00;         				 // 5) 4 OUTPUT , 4 INPUT  
  GPIOA->AFSEL = 0x00;      				 // 6) no alternate function
  ...
  GPIOA->DEN = 0xFF;       				   // 7) enable digital pins PF4-PF0
	...
	GPIOA->IM |=0xFF;      // Unmask the interrupts for PF0 and PF4
	GPIOA->IS |= 0xFF;     // Make bits PF0 and PF4 level sensitive
	GPIOA->IEV &= ~0xFF;   // Sense on Low Level
